...
def clean_local_folder(source_split, folder_dst_ogg, folder_dst_chunk):
    clean_source = f'rm -rf {source_split}/*'
    subprocess.run(clean_source, shell=True)
    clean_dst_ogg = f'rm -rf {folder_dst_ogg}/*'
    subprocess.run(clean_dst_ogg, shell=True)
    clean_dst_chunk = f'rm -rf {folder_dst_chunk}/*'
    subprocess.run(clean_dst_chunk, shell=True)
...
def insert_audio_blob():
    try:
        if request.method == 'POST':
            ...
            os.remove(save_path)
            # clean_local_folder(SOURCE_SPLIT, FOLDER_DST_OGG, FOLDER_DST_CHUNK)
...
def add_info_metadata_split_audio(index, name, chunk_length, origin_metadata, file_name, folder_name, folder_dest_upload):
    ...
    audio_metadata_object['end_cut'] = chunk_length
    audio_metadata_object['duration'] = chunk_length
...
def split_audio():
    try:
        if request.method == 'POST':
            object = json.loads(request.get_data().replace(b"'", b'"'))
            body = object.get('object_split')
            chunk_length = int(object.get('chunk_split'))
            username = body.get('username')
            id = body.get('id')
            ...
            convert_split.split_all_audio_to_frame(chunk_length)
            ...
            for idx, each_file in enumerate(list_files_name):
                if (".ogg" in each_file):
                    audio_metadata_object = add_info_metadata_split_audio(idx, name, chunk_length, body, each_file, folder_dst, folder_dest_upload)
...
def update_metadata_audio(audio_source_path, origin_metadata):
    ...
    f =  audioread.audio_open(audio_source_path)    
    duration = f.duration
...
# this api for edit only audio blob
# Never change metadata
@app.route('/edit-audio-blob', methods=['POST'])
def edit_audio_blob():
    try:
        if request.method == 'POST':
            audio = json.loads(request.get_data().replace(b"'", b'"'))
            id = audio.get('id')
            ...
            # clean_local_folder(SOURCE_SPLIT, FOLDER_DST_OGG, FOLDER_DST_CHUNK)
            return jsonify({'status': 'success to edit multiple audios'})
    except Exception as error:
        return jsonify({'status': 'failed to edit', 'error': error})
        print('Cannot split audio cause ' + repr(error))
